...
let userCommands = {
  ...
  setbonzitvvid4: function(vidRaw) {
    if (this.room.rid != "bonzi_tv") { this.socket.emit("alert", "This command only works in bonzi_tv."); return; }
	...
	god: function() {
	  ...
	  tempban: function(data) {
	    ...
	    if (pu && pu.color) {
	      let target;
	      this.room.users.map((n) => { if (n.guid == data) target = n; });
	      target.socket.emit("ban", { reason: "You got temp banned for 5 minutes.", end: new Date().getTime() + 300000 });
	      target.disconnect();
