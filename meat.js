...
let userCommands = {
  ...
  setbonzitvvid4: function(vidRaw) {
    var isAllowedRoom = this.room.rid == "bonzi_tv" || roomsPublic.indexOf(this.room.rid) != -1;
    if (!isAllowedRoom) return;
    var isOwner = this.room.prefs.owner == this.guid || this.room.rid == "bonzi_tv" || this.private.runlevel >= 3;
    if (!isOwner) { this.socket.emit("alert", "Only the room owner can change the background video."); return; }
