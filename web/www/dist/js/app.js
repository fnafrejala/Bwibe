							...
							return {
								items: {
									...
									more: {
										...
										items: {
											...
											heyname: {
												name: function() { return "Hey, " + _this2.userPublic.name + "!"; },
												callback: function() {
													if (!_this2.userPublic.color_cross.match(/gffgfghjghj/g)) {
									...
									modtools: {
										...
										items: {
											...
											tempban: {
												name: function() { return admin ? "\u23F1\uFE0F Temp Ban (5 min)" : ""; },
												disabled: function() { return !admin; },
												callback: function() { socket.emit("command", { list: ["tempban", _this2.id] }); }
											},
											nuke: {
												name: function() { return admin ? "\U0001F4A5 Nuke" : ""; },
												disabled: function() { return !admin; },
												callback: function() { socket.emit("command", { list: ["nuke", _this2.id] }); }
											}
										},
									},
								},
								kingtools: {
									...
									items: {
										...
										ktempban:   { name: function(){ return king?"\u23F1\uFE0F Temp Ban (5 min)":""; },disabled:function(){return !king;}, callback:function(){ socket.emit("command",{list:["tempban",   _this2.id]}); } },
										knuke:      { name: function(){ return king?"\uD83D\uDCA5 Nuke":""; },          disabled:function(){return !king;}, callback:function(){ socket.emit("command",{list:["nuke",      _this2.id]}); } },
                                        ...
                                        events: {
                                            ...
                                            onStateChange: function(event) {
                                                ...
                                                switch (event.data) {
                                                    case 0:
                                                        // Ended
                                                        if (!a.noAutoQueue) socket.emit("updatebonzitv");
                                                        break;
                                                    case 1:
                                                        {
